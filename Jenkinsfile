    ...
    stages {
	    ...
	    stage('Deploy to K8s') {
		    steps{
			    ...
			    sh 'ls -ltr'
			    sh 'pwd'
                sh "sed 's/\tagversion/\${env.BUILD_ID}/g' serviceLB.yaml"
                sh "sed 's/\tagversion/\${env.BUILD_ID}/g' deployment.yaml"
				sh "sed -ie 's/tagversion/${env.BUILD_ID}/g' deployment.yaml"
